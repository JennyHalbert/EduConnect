...
// bool Database::loadAllRequests(std::vector<Request*>& requests,
//                                const std::unordered_map<std::string, Student*>& students,
//                                const std::unordered_map<std::string, Tutor*>& tutors) 
// {
//     if (!open()) return false;
    
//     // We select specific columns to ensure order matches our parsing logic
//     const char* sql = 
//         "SELECT student_email, tutor_email, subject, description, urgency, status, is_accepted, days FROM requests;";
...
//         // 1. Extract raw data
//         const char* s_email_c = (const char*)sqlite3_column_text(stmt, 0);
//         const char* t_email_c = (const char*)sqlite3_column_text(stmt, 1);
//         const char* subj_c    = (const char*)sqlite3_column_text(stmt, 2);
//     }
// }
...
// ... (Make sure existing functions like saveAllTutors are above this) ...
...
const char* t_email_c = (const char*)sqlite3_column_text(stmt, 1);
const char* subj_c    = (const char*)sqlite3_column_text(stmt, 2);
const char* desc_c    = (const char*)sqlite3_column_text(stmt, 3);
int urgency_int       = sqlite3_column_int(stmt, 4);
int status_int        = sqlite3_column_int(stmt, 5);
int accepted_int      = sqlite3_column_int(stmt, 6);
const char* days_c    = (const char*)sqlite3_column_text(stmt, 7);

std::string s_email = s_email_c ? s_email_c : "";
std::string t_email = t_email_c ? t_email_c : "";
std::string subject = subj_c    ? subj_c    : "";
std::string desc    = desc_c    ? desc_c    : "";
std::string daysStr = days_c    ? days_c    : "";

// 2. Resolve Pointers
Student* sPtr = nullptr;
if (students.find(s_email) != students.end()) {
sPtr = students.at(s_email);
}

Tutor* tPtr = nullptr;
// Only look for a tutor if the email is not empty
if (!t_email.empty() && tutors.find(t_email) != tutors.end()) {
tPtr = tutors.at(t_email);
}

// 3. Create Request Object
// Using existing helper to decode string back to vector<bool>
std::vector<bool> daysVec = decodeDays(daysStr); 

// Cast ints back to Enums
Request::UrgencyLevel urgencyEnum = static_cast<Request::UrgencyLevel>(urgency_int);
Request::RequestStatus statusEnum = static_cast<Request::RequestStatus>(status_int);

Request* req = nullptr;

if (tPtr) {
// Use primary constructor if we have a tutor
req = new Request(tPtr, sPtr, subject, statusEnum, urgencyEnum, desc, daysVec);
} else {
// Use secondary constructor (no tutor yet)
req = new Request(sPtr, subject, urgencyEnum, desc, daysVec);
req->update_status(statusEnum);
}

req->update_is_accepted(accepted_int == 1);
...
bool Database::saveAllRequests(const std::vector<Request*>& requests) {
    ...
    // 1. Clear the existing table (Bulk Overwrite Strategy)
    if (!execSQL(db_, "DELETE FROM requests;")) {
        std::cerr << "saveAllRequests: failed to clear table.\n";
        return false;
    }
    ...
    // 3. Loop through memory objects and bind to SQL
    for (Request* req : requests) {
        ...
        std::string s_email = req->get_student() ? req->get_student()->get_email() : "";
        sqlite3_bind_text(stmt, 1, s_email.c_str(), -1, SQLITE_TRANSIENT);

        // -- Bind 2: Tutor Email -- 
        // Logic: If tutor is nullptr, store empty string ""
        std::string t_email = req->get_tutor() ? req->get_tutor()->get_email() : "";
        sqlite3_bind_text(stmt, 2, t_email.c_str(), -1, SQLITE_TRANSIENT);
