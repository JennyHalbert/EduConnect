...
public:
    ...
    // load in request 
    // In Database.h, inside the public section:

// ... existing declarations ...

// Save all requests from a vector to the DB



        // Load all requests from DB into a vector
// Note: Needs access to the student/tutor maps to resolve pointers!
    bool loadAllRequests(std::vector<Request*>& requests,
    const std::unordered_map<std::string, Student*>& students,
    const std::unordered_map<std::string, Tutor*>& tutors);

    bool saveAllRequests(const std::vector<Request*>& requests);
